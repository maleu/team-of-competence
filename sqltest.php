...
if (mysqli_connect_errno() == 0) {
	$sql = "SELECT ArtikelID, Name, Einkaufspreis, Lieferant, Bestand, Verkaufspreis FROM Artikel";
	// select beispiel
	...
	if ($num_rows !== 0) {
		$result -> bind_result($ArtikelID, $Name, $Einkaufspreis, $Lieferant, $Bestand, $Verkaufspreis);
		while ($result -> fetch()) {
			echo $ArtikelID.', '.$Name.', '.$Einkaufspreis.', '.$Lieferant.', '.$Bestand.', '.$Verkaufspreis;
		}

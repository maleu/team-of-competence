...
if (mysqli_connect_errno() == 0) {
	if ($prodName != "Produktnamen eingeben..."){
		$sql = "SELECT ArtikelID, Name, Verkaufspreis, Lieferant, Bestand FROM Artikel WHERE Name LIKE '%$prodName%'";
	}elseif ($prodNummer != "Produktnummer eingeben..."){	
		$sql = "SELECT ArtikelID, Name, Verkaufspreis, Lieferant, Bestand FROM Artikel WHERE ArtikelID = $prodNummer";
	}else{
		exit;
	}
		...
		while ($result -> fetch()) {
			echo'
				<div class=lagerBestellungTabelleZeile>
					...
					<div class=lagerBestellungTabelleBElement>
						<input type="button" id="einkaufUebernehmen'.$i.'" value="Bestellen" onclick="bestellungUebernehmen('.$i.')" />
					</div>
				</div>
			';
			$i++;
